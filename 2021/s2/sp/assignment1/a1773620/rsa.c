...
int i_bit( num_t n, int i ) {
  /* Convert num_t to hexadecimal string */
  char* str = num_toString( n ) ;

  /* get the character containing the ith bit */
  ...
  /* Get the correct index from the binary string */
  i = i % 4 ;
  /* Return 1 or 0 depending on bit value */
  return binary[3 - i] - 48 ; /* -48 converts char to int for decimal 0-9 */
}
...
{
    bignum_t temp ;
    bignum_t dc ; /* Don't care variable */
    ...
    for( int i = exp_size * 4 - 1 ; i >= 0 ; i-- ) {
        ...
        /* Check if the ith bit of exp is 1 */
        if( i_bit( exp, i ) == 1 ) {
          /* Multiply */
...
int rsa_pkcs_encrypt(
    ...
    const num_t n ) {
      ...
      /* Generate random bits of hexadecimal to pad message to 1024 bits long */
      srand( time( 0 ) ) ;
      /* Allocate memory for the random string */
      char *r = calloc( ( 251 - messagelen ), sizeof(char) ) ;
      for( i = 0 ; i < 250 - messagelen ; i++ )
      {
        char temp[2] ;
        /* Generate a random non-zero hexadecimal number */
        sprintf( temp, "%X", rand() % 15 + 1 ) ;
        /* Add it to the end of r */
        strcat( r, temp ) ;
      }

      /* Add the random padding to the message */
      strcat( str, r ) ;
      free( r ) ;

      /* Add the zero delimeter and the original message */
      strcat( str, "00" ) ;
      strcat( str, message ) ;
