/* a1773463, Jones, Leo
// a1773620, Rodda, Matthew
// aXXXXXXX, Mastersson, Alex
// jpr */
...
/* c00 1 8 72
ID, prio, arrival time, play time */
...
int main(int argc, char *argv[])
{
    ...
    for (int current_time = 0; !all_done; current_time++)
    {
        ...
        {
            // Get ID of the new customer
            int arrival_id = arrival_events[0].customer_id ;
            // Get remaining time of new customer
            int arrival_slots_remaining = customers[arrival_id].slots_remaining ;
            // Get priority of new customer
            int arrival_priority = customers[arrival_id].priority ;

            std::deque<int>::iterator it = queue.begin() ;
            for( int i = 0 ; i < queue.size() ; i++ ) {
                if( customers[queue[i]].priority == arrival_priority &&
                    customers[queue[i]].slots_remaining > arrival_slots_remaining ) {
                        queue.insert( it, arrival_id ) ;
                        break ;
                    }
                ++it ;
            }

            // If we got to the end without adding an element
            if( it == queue.end() ) {
                it = queue.begin() ;

                for( int i = 0 ; i < queue.size() ; i++ ) {
                    if( customers[queue[i]].priority == arrival_priority ) {
                        for( int j = i ; j < queue.size() && customers[queue[j]].priority == arrival_priority ; j++ ) {
                            ++it ;
                        }
                        queue.insert( it, arrival_id ) ;
                        break ;
            ...
            if( it == queue.end() ) {
                if( arrival_priority == 0 ) queue.push_front( arrival_id ) ;
                if( arrival_priority == 1 ) queue.push_back( arrival_id ) ;
            }

            if( queue.empty() ) queue.push_back( arrival_id ) ;

            // Print queue
            // for( int i = 0 ; i < queue.size() ; i++ ) {
            //     std::cout << customers[queue[i]].priority << ":"<< queue[i] << ":" << customers[queue[i]].slots_remaining << " " ;
            // }
            // std::cout << std::endl ;
            arrival_events.pop_front();
        }
        // std::cout << "Made it out of adding elements" << std::endl ;
        ...
        if (current_id >= 0)
        {
            if (current_time == time_out)
            {
                ...
                if (customers[current_id].slots_remaining > 0)
                {
                    ...
                    std::cout << "Put customer " << current_id << " back into queue with slots left = " << customers[current_id].slots_remaining << std::endl ;
                }
        ...
        if (current_id == -1)
        {
            ...
            {
                std::cout << "Queue was not empty!" << std::endl ;
