...
/* Find the distance bewteen two routers regardless of the firstHop
 * Loop through all the routingTables, then all their links
 * This time ONLY check if the destination matches
 * If the loop finishes without finding a link, return NO_LINK
 * This function works off the assumption that links will start at routerName
 */
{
	for( int i = 0 ; i < vec->size() ; i++ ) {
		/* Check if the name of the routingTable matches */
 ...
 * Use the broadcast vector for the remaining trip distance
			...
			if( i != j ) {
				...
				for( int k = 0 ; k < neighbours->size() ; k++ ) { /* loop over each final destination */ 
					if( i != k ) {
						...
						/* This time we don't care where our first hop is */
